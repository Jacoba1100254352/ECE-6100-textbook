...
\begin{itemize}
    \item $P_0$ through $P_3$ compete first on one local lock,
    \item $P_4$ through $P_7$ compete on another,
    \item $P_8$ through $P_{11}$ and $P_{12}$ through $P_{15}$ do the same,
    \item and then only the local winners advance to higher-level locks.
\end{itemize}
That structure reduces the invalidation flood at the top of the tree because not all 16 contenders pound on the same variable at the same instant.
