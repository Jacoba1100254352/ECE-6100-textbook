...
\begin{itemize}
    ...
    \item \texttt{Act}: acknowledgment that a requested action completed.
...
The directory sets $P_2$\('\)s presence bit and returns the block so that $P_2$ enters \texttt{S}.
...
Only after every required acknowledgment has arrived does the directory send the data and permission to $P_2$, clear the other presence bits, set $P_2$\('\)s bit, and mark the entry dirty.
...
One useful way to reason about homework-style metadata questions is therefore:
\begin{enumerate}
