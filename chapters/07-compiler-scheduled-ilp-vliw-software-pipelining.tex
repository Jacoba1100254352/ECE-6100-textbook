...
One response to dynamic-scheduling complexity is to shift more of the scheduling work into the compiler.
As superscalar windows grow, the hardware would like a larger scheduling queue so it can search farther ahead for independent work, but larger queues also make wakeup, select, and operand-readiness checks harder to close at a short cycle time.
This tradeoff motivates the \textit{very long instruction word (VLIW)} approach, historically associated with Josh Fisher.
In a VLIW machine, one instruction word encodes several operations intended to execute in parallel, so the compiler determines the schedule at compile time rather than leaving that work to large runtime scheduling hardware.
...
The tradeoff is that the compiler must do much more work up front, and the resulting code is less adaptable to runtime variation such as cache misses or changing branch behavior.
...
Because the optimal schedule for a dependence-constrained block is exponential in block size, practical VLIW compilers rely on heuristics rather than exact search.
...
The compiler begins with a \textit{data dependence graph}.
...
Draw one node per op, then add all true flow edges from each definition to the later uses that require that value.
...
\end{table}
Figure~\ref{fig:ilp-vliw-ddg} shows the same block in a more visual form: the original operations, the resulting data dependence graph, and one priority list ordered by dependence height.
\begin{figure}[t]
    \centering
    \small
...
The ready list is obtained by sorting nodes from highest height to lowest and breaking ties either arbitrarily or by an explicit secondary rule such as program order.
...
List scheduling then places the highest-priority ready instructions into each cycle, subject to both dependence timing and resource availability.
...
The point is not that this schedule is unique, but that the compiler can choose the cycle packing in advance and encode it directly.
...
\end{table}
The schedule highlights two points.
...
Second, the absence of an operation in a slot does not require the hardware to infer what to do dynamically; the compiler has already encoded the intended schedule.
...
VLIW raises an encoding question: how should a machine represent the boundaries of a multi-op when not every cycle uses every functional unit?
...
The following op then begins a new multi-op.
...
This is a compact way to represent silence between multi-ops while keeping each op's encoding regular.
...
That saves encoding space, but it also gives up some of the compiler's direct control over exactly when the following multi-op may begin.
...
Conceptually, the compiler behaves as if it had unrolled the loop many times, compacted the overlapping schedule, and then retained only the repeating pattern.
...
\paragraph{Unrolling view before compaction}\label{par:ilp-swpipe-unroll-view}
One useful way to understand software pipelining is to look first at a small explicit unrolling.
Suppose a loop body consists of five ops $A,B,C,D,E$, where $E$ is the loop-closing branch.
A two-times unrolled form can be sketched conceptually as
\[
    A,\ B,\ C,\ D,\ E^\ast,\ A',\ B',\ C',\ D',\ E',
\]
where $E^\ast$ is the inverted early-exit branch that leaves the loop if the second copy should not begin.
This makes the scheduling opportunity visible: once the loop is unrolled, the compiler can interleave $A'$ with still-pending work from the first copy instead of waiting for the whole first iteration to drain first.
Software pipelining then takes the next step and \emph{compacts} that overlapping pattern into a finite prologue, a repeating kernel, and a finite epilogue rather than materializing a large explicitly unrolled body.

\paragraph{Which dependences matter most in the first sketch}\label{par:ilp-swpipe-dep-simplify}
In early cyclic-scheduling sketches, it is common to draw only true dependences and the loop-carried recurrences that constrain them.
Anti-dependence and output-dependence edges are still real constraints in the full compiler problem, but they are often omitted from the first visualization because software renaming can frequently remove them.
The scheduling bottlenecks that determine $RecII$ come from the true cyclic constraints, so those are the ones worth emphasizing first.


\section{Iterative Modulo Scheduling}\label{sec:ilp-iterative-modulo-scheduling}
A practical way to construct these schedules is \textit{Iterative Modulo Scheduling (IMS)}.
...
\paragraph{Review Emphasis: Why IMS Starts at a Lower Bound}\label{par:ch7-review-ims-lower-bound}
The naive version of ``try every initiation interval'' can become too expensive because each scheduling attempt is itself a scheduling problem.
List scheduling is often treated as roughly $O(n^2)$ in the number of operations, so repeatedly trying many impossible values of $II$ can push the compile-time cost much higher.
The point of $ResII$ and $RecII$ is to skip initiation intervals that cannot possibly work.
In practice, a conservative lower bound usually lets IMS succeed after only a small number of attempts, while still guaranteeing eventual success once $II$ becomes large enough.

In most textbook treatments, the underlying machine is modeled with a point-latency abstraction: an op occupies one issue slot, and its result becomes ready after a fixed latency.
...
The following loop illustrates the idea.
...
One legal modulo reservation table appears in Table~\ref{tab:ilp-swpipe-kernel}.
...
At the same time, general-purpose superscalar machines still benefit from dynamic scheduling because real loads do not always return in a fixed number of cycles and because binaries may need to run across several microarchitectural variants.

\paragraph{Review Emphasis: Prologue, Kernel, and Epilogue}\label{par:ch7-review-prologue-kernel-epilogue}
The prologue is the startup code needed before the repeating pattern is fully populated.
The kernel is the compact repeated pattern that behaves like many overlapped loop iterations.
The epilogue drains work that was started by the final kernel iterations but has not yet completed.
If nothing wraps across the compact kernel boundary, the prologue or epilogue may become trivial, but the conceptual split is still useful because it explains how a rerolled schedule preserves the behavior of the unrolled loop.
...
The compiler therefore needs \textit{memory disambiguation}: it must prove that two memory references do not overlap, or else preserve the original order or insert explicit synchronization.
...
Chapter 7 presents compiler-scheduled ILP as an alternative to increasingly aggressive dynamic superscalar scheduling.
...
\section{Practice Questions}\label{sec:ch7-practice}
The following questions reinforce the main ideas of Chapter 7 and are intended for self-checking after the summary.
\begin{enumerate}
    \item For the sequence \texttt{LW R1,X}; \texttt{ADD R2,R1,R3}; \texttt{ADD R1,R4,R5}; \texttt{MUL R6,R2,R1}, draw the data dependence graph and label the pure, anti-, and output dependences.
    \item Using appropriate latencies for load, add, and multiply, compute the dependence height of each instruction in the previous question.
    Then list the instructions in descending order of dependence height.
    \item Assume a VLIW machine with at most one ALU op, one MUL op, and one memory op per multi-op.
    Produce one legal list schedule for the instruction sequence in Question 1.
    \item A VLIW schedule contains the following multi-ops: row 1 has a single load; row 2 has an add and a multiply; row 3 is empty because of a dependence delay; row 4 has a store.
    Explain how a tail-bit plus pause-field encoding can represent this schedule without inserting explicit no-op instructions into every unused slot.
    \item A loop contains 3 ALU/branch operations, 1 multiply, and 3 memory operations per iteration.
    The machine has 1 ALU, 1 MUL, and 1 memory unit.
    Compute $ResII$.
    Then explain how a loop-carried recurrence could still force $MII$ above that resource bound.
    \item For the loop in Question 5, suppose the strongest loop-carried recurrence has total latency 4 and distance 1.
    Compute $RecII$ and $MII$.
    \item Explain why a modulo reservation table for initiation interval $II$ has exactly $II$ rows, and explain the role of the mapping from cycle $x$ in the unrolled schedule to row $x \bmod II$.
    \item Why does IMS begin at $MII$ instead of always starting at $II=1$? Explain the compile-time cost argument and the correctness argument.
    \item In a software-pipelined loop, what work belongs in the prologue, what work belongs in the kernel, and what work belongs in the epilogue?
    \item Under what conditions may a compiler legally move a load ahead of an older store?
    Explain why this question is fundamentally a memory-disambiguation problem rather than only a functional-unit scheduling problem.
\end{enumerate}
