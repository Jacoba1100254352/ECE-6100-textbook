...
\begin{equation}\label{eq:appd-phase-oracle}
    U_f\lvert x\rangle = (-1)^{f(x)}\lvert x\rangle \cdot
\end{equation}
...
This is not generic magic parallelism: the algorithm must match the mathematical structure of the problem.
...
\begin{enumerate}
    \item Quantum computers are accelerators coordinated by classical computers, not drop-in replacements for all classical processors.
